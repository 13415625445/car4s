...
(function() {
    ...
    }), pageConfig.TPL_MSlide = {itemsWrap: '<ul class="slide-items">{innerHTML}</ul>',itemsContent: '{for item in json}            {var v=pageConfig.FN_GetCompatibleData(item)}            <li class="hide" clstag="homepage|keycount|home2013|09a${parseInt(item_index)+2}" style="height: 402px"><a href="${v.href}" target="_blank" title="${v.alt}" style="height: 402px;">                <img height="${v.height}" width="${v.width}" src="${v.src}" data-img="2" style="height: 402px;" />            </a></li>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for item in json}            <span class="{if parseInt(item_index)==0}curr{/if}">${parseInt(item_index)+1}</span>        {/for}'}, pageConfig.TPL_FSlide = {itemsWrap: '<div class="slide-itemswrap"><div class="slide-items">{innerHTML}</div></div>',itemsContent: '{for item in json}            <div><a href="${item.href}" target="_blank"><img src="${item.src}" width="${item.width}" height="${item.height}" alt="${item.alt}" data-img="2"></a></div>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for item in json}            <span class="{if parseInt(item_index)==0}curr{/if}"><b></b></span>        {/for}'}, pageConfig.FN_SetPrice = function(t) {
        var e = t.split(".");
